...
#define SHARE_DOCUMENT      kSharedContainerUrl.path
...
static NSString *const DEFAULT_DB_NAME = @"kv.db";
...
- (id)initDBWithName:(NSString *)dbName {
    self = [super init];
    if (self) {
        NSString * dbPath = [SHARE_DOCUMENT stringByAppendingPathComponent:dbName];
...
- (void)createTableWithName:(NSString *)tableName {
    if ([KVStore checkTableName:tableName] == NO) {
        return;
    }
...
- (id)getObjectById:(NSString *)objectId fromTable:(NSString *)tableName {
    KVItem * item = [self getKVItemById:objectId fromTable:tableName];
    if (item) {
