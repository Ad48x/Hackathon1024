...
{
    if ([self selectedCount] == 5 && !btn.checked) {
        [PINToast showMessage:@"最多选 5 个标签"];
        return ;
    }
    
    btn.checked = !btn.checked;
    
    if (self.didClickTagAtIndex && btn.checked) {
        self.didClickTagAtIndex([self.subviews indexOfObject:btn], btn.currentTitle, btn.checked);
    }
}
